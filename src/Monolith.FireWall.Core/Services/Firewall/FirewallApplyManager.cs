...
public sealed class FirewallApplyManager
{
    ...
    private readonly FirewallInterfaceIntegration? _interfaceIntegration;
        ...
        FirewallInterfaceIntegration? interfaceIntegration = null)
    {
        ...
        _interfaceIntegration = interfaceIntegration;
    ...
    public async Task<FirewallApplyResult> ApplyAsync(CancellationToken cancellationToken)
    {
        ...
        // STEP 4: Sync dynamic gateways after firewall apply
        // This ensures DHCP gateways are detected and available in the gateway list
        try
        {
            // Note: We don't have direct access to RoutingManager here, but gateway sync
            // happens automatically when GetGatewaysAsync is called, which is fine.
            // The gateway sync service also runs periodically in the background.
        }
        catch
        {
            // Don't fail firewall apply if gateway sync fails
        }
    ...
    public async Task<FirewallApplyResult> BuildConfigAsync(CancellationToken cancellationToken)
    {
        ...
        // Resolve dynamic aliases (e.g., $wan_ip, $lan_subnet) and add them to the alias list
        if (_interfaceIntegration != null)
        {
            try
            {
                var dynamicAliases = await _interfaceIntegration.ResolveAllAliasesAsync();
                foreach (var kvp in dynamicAliases)
                {
                    // Add dynamic aliases as "hosts" type aliases with single resolved value
                    aliases.Add(new FirewallAliasView
                    {
                        Name = kvp.Key,
                        Type = kvp.Value.Contains('/') ? "networks" : "hosts",
                        Content = new List<string> { kvp.Value },
                        Description = $"Dynamic alias (auto-resolved)",
                        Enabled = true
                    });
                }
            }
            ...
            {
                warnings.Add($"Failed to resolve dynamic aliases: {ex.Message}");
            }
        }
